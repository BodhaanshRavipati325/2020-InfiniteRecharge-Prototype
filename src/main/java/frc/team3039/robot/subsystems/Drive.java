...
public class Drive extends Subsystem {
	...
	public enum DriveControlMode {
		JOYSTICK, HOLD, PATH_FOLLOWING, OPEN_LOOP
	}
	...
	private TalonSRXEncoder leftDrive1, rightDrive1;
	private TalonSRX leftDrive2, rightDrive2, leftDrive3, rightDrive3;
	...
	public static final double STEER_NON_LINEARITY = 0.5;	//Max Turning Output
	public static final double MOVE_NON_LINEARITY = 1.0; 	//Max Drive Output

	public static final double STICK_DEADZONE = 0.02;		//Dead zone for Joysticks on the controller

	public static final double PITCH_THRESHOLD = 20;		//Limit Robot Pitch Before Anti-Tip Code Runs
	...
	//MP Setup
	private MPTalonPIDController mMPTalonPIDController;
	...
	private Drive() {
		try {
			...
			mShifter = new Solenoid(RobotMap.DRIVETAIN_SHIFT_PCM_ID);
	...
	public synchronized void driveWithJoystick() {
		...
		m_moveInput = RobotContainer.getInstance().getDriverController().getLeftYAxis();
		m_steerInput = -RobotContainer.getInstance().getDriverController().getRightXAxis();
		...
		boolean mShiftDrive = RobotContainer.getInstance().getDriverController().getRightTrigger().get();
		...
		if (Math.abs(pitchAngle) > PITCH_THRESHOLD) {
			...
			m_steerOutput = 0;
			System.out.println("Pitch Threshold 2 angle = " + pitchAngle);
	...
	public void setDriveHold(boolean setHold) {
		if (setHold) {
	...
	public void driveHold(){
		mMPTalonPIDController.setPID(mpHoldPIDParams, kPositionControlSlot); //TODO Check
